...
let mainWindow = null;
let ElectronVideoProcessor = null;
...
async function loadVideoProcessor() {
  try {
    const module = await import('../dist-electron/main/videoProcessor.js');
    ElectronVideoProcessor = module.ElectronVideoProcessor;
  } catch (error) {
...
app.whenReady().then(async () => {
  // Load and initialize video processor
  await loadVideoProcessor();
  if (ElectronVideoProcessor) {
